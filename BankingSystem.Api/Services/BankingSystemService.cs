    ...
    public class BankingSystemService: Banking.BankingBase
    {
        ...
        public override async Task<TransferResponse> Transfer(TransferRequest request, ServerCallContext context)
        {
            try
            {
                ...
                if( !_currencyRateService.TryGetRate(accountFrom.Balance.Currency, accountTo.Balance.Currency, out var currencyRate))
                {
                    throw new Exception($"Нет курса {accountFrom.Balance.Currency} к {accountTo.Balance.Currency}");
                }

                accountTo.Transfer(accountTo, new Money(decimal.Parse(request.Money.Amount) * currencyRate, request.Money.Currency));
